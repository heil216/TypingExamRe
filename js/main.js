...

{
  ...
  let currentIndex = 0;
  const startTime = 5;//キーダウンから実験開始までの時間=5
  const ready_time_limit = 15;//課題開始までの時間=15
  const typing_time_limit = 20;//タイピング課題時間=20
  const remain_time_limit = 20;//安静時間=20
  const trialConut = 8;//試行回数=8
  ...
  document.addEventListener('keydown',() => {
    ...
    isPlaying = true ;
    target.textContent = "5秒後に開始します。キーボードに　手を置いてなるべく動かないように　してお待ち下さい。"
    setTimeout(function(){
      ready();
    },(startTime * 1000) - 1000);
  });
